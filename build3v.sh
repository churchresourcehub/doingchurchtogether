...
fadescene () {
  ...
  ffmpeg -y -loglevel error \
    ...
    -loop 1 -t "$DUR" -i "$OV" \
    -filter_complex "\
[0:v]fps=$FPS,tpad=stop_mode=clone:stop_duration=$DUR,trim=duration=$DUR,scale=1080:1920,setsar=1[bg];\
...
fadescene vseq-landing 16 vw-open.png 0.7 5.0 x1.mp4
# Scene 2: hub type+scroll (30f @ 7fps = 4.3s bg), 9.0s total, captions switch at 4.6
cardscene vseq-hub 7 vw-hubA.png vw-hubB.png 9.0 4.6 x2.mp4
# Scene 3: hold on the Triangle, click The Church House, slow scroll
cardscene vseq-book 8 vw-bookA.png vw-bookB.png 9.8 5.4 x3.mp4
# Scene 4 (plays fifth, the case-study coda): bot typing + answer, 10.5s, switch 5.2
cardscene vseq-bot 6 vw-botA.png vw-botB.png 10.5 5.2 x4.mp4
# Scene 5 (plays fourth): comparative clicks, 9.0s, switch 4.6
cardscene vseq-comp 3.5 vw-compA.png vw-compB.png 9.0 4.6 x5.mp4
# Scene 6: outro typed URL over hero
typedscene vseq-landing 40 vtseq-out 13 5.0 x6.mp4

D1=$(ffprobe -v error -show_entries format=duration -of csv=p=0 x1.mp4)
D2=$(ffprobe -v error -show_entries format=duration -of csv=p=0 x2.mp4)
D3=$(ffprobe -v error -show_entries format=duration -of csv=p=0 x3.mp4)
D4=$(ffprobe -v error -show_entries format=duration -of csv=p=0 x5.mp4)
D5=$(ffprobe -v error -show_entries format=duration -of csv=p=0 x4.mp4)
...
ffmpeg -y -loglevel error \
  -i x1.mp4 -i x2.mp4 -i x3.mp4 -i x5.mp4 -i x4.mp4 -i x6.mp4 \
  -filter_complex "\
...
[d][5:v]xfade=transition=fade:duration=$TD:offset=$O5,format=yuv420p[v]" \
  ...
  DoingChurchTogether-promo-v3-vertical.mp4

echo "--- final ---"
ffprobe -v error -show_entries format=duration,size -of default=noprint_wrappers=1 DoingChurchTogether-promo-v3-vertical.mp4
